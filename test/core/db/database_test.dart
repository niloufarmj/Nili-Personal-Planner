...
void main() {
  ...
  test('day_tags CRUD', () async {
    ...
    await db.into(db.dayTags).insert(
          DayTagsCompanion.insert(date: '2026-01-15', tagId: tagId),
        );
  ...
  test('events CRUD', () async {
    ...
    final row =
        await (db.select(db.events)..where((e) => e.id.equals(id))).getSingle();
  ...
  test('reminders CRUD with JSON notifyRule', () async {
    ...
    final row = await (db.select(db.reminders)..where((r) => r.id.equals(id)))
        .getSingle();
  ...
  test('collections, items, subtasks CRUD', () async {
    final colId = await db.into(db.collections).insert(
    ...
    await db.into(db.subtasks).insert(
          SubtasksCompanion.insert(itemId: itemId, title: 'Red variety'),
        );

    final col = await (db.select(db.collections)
          ..where((c) => c.id.equals(colId)))
        .getSingle();
    expect(col.template, 'shopping');
    expect(col.archived, false);

    final item = await (db.select(db.items)..where((i) => i.id.equals(itemId)))
        .getSingle();
    expect(item.title, 'Apples');
    expect(item.meta!['shop_url'], 'https://shop.example.com');

    final subs = await (db.select(db.subtasks)
  ...
  test('transactions CRUD', () async {
    ...
    final row = await (db.select(db.transactions)..where((t) => t.id.equals(id)))
        .getSingle();
  ...
  test('debts CRUD', () async {
    final id = await db.into(db.debts).insert(
    ...
    final row =
        await (db.select(db.debts)..where((d) => d.id.equals(id))).getSingle();
  ...
  test('ingredients, recipes, recipe_ingredients, meal_slots CRUD', () async {
    ...
    final recId = await db.into(db.recipes).insert(
    ...
    await db.into(db.mealSlots).insert(
    ...
    final recipe = await (db.select(db.recipes)..where((r) => r.id.equals(recId)))
        .getSingle();
    expect(recipe.tags, contains('high-protein'));

    final ri = await (db.select(db.recipeIngredients)
          ..where((ri) => ri.recipeId.equals(recId)))
        .getSingle();
    expect(ri.amount, 200);

    final slot = await (db.select(db.mealSlots)
          ..where((s) => s.slot.equals('dinner')))
        .getSingle();
  ...
  test('workout_plans, gym_sessions CRUD', () async {
    final planId = await db.into(db.workoutPlans).insert(
          WorkoutPlansCompanion.insert(
            name: 'Plan A',
            content: '## Plan A\n- Squats',
          ),
        );
    final sessId = await db.into(db.gymSessions).insert(
    ...
    final sess = await (db.select(db.gymSessions)
          ..where((s) => s.id.equals(sessId)))
        .getSingle();
    expect(sess.status, 'done');
  });

  test('measurements CRUD with JSON converters', () async {
    final id = await db.into(db.measurements).insert(
    ...
    final row = await (db.select(db.measurements)..where((m) => m.id.equals(id)))
        .getSingle();
  ...
  test('fitness_goals CRUD', () async {
    final id = await db.into(db.fitnessGoals).insert(
          FitnessGoalsCompanion.insert(metric: 'weight_kg', target: 58.0),
        );
    final row = await (db.select(db.fitnessGoals)..where((g) => g.id.equals(id)))
        .getSingle();
  ...
  test('habits, habit_logs CRUD with JSON reminderTimes', () async {
    final habitId = await db.into(db.habits).insert(
          HabitsCompanion.insert(
            ...
            reminderTimes:
                const Value(['09:00', '13:00', '17:00', '21:00']),
          ),
        );
    ...
    final habit =
        await (db.select(db.habits)..where((h) => h.id.equals(habitId)))
            .getSingle();
  ...
  test('wellbeing_actions, wellbeing_logs CRUD', () async {
    final actionId = await db.into(db.wellbeingActions).insert(
          WellbeingActionsCompanion.insert(name: 'Meditation'),
        );
    await db.into(db.wellbeingLogs).insert(
  ...
  test('work_contexts, time_entries CRUD', () async {
    final ctxId = await db.into(db.workContexts).insert(
          WorkContextsCompanion.insert(name: 'FreshFX'),
        );
  ...
  test('social_accounts, social_logs CRUD', () async {
    final accId = await db.into(db.socialAccounts).insert(
          SocialAccountsCompanion.insert(platform: 'Instagram'),
        );
    final logId = await db.into(db.socialLogs).insert(
    ...
    final log =
        await (db.select(db.socialLogs)..where((l) => l.id.equals(logId)))
            .getSingle();
  ...
  test('app_settings CRUD', () async {
    ...
    final row = await (db.select(db.appSettings)
          ..where((s) => s.key.equals('calendar_filter')))
        .getSingle();
  ...
  test('StringListConverter round-trips correctly', () async {
    ...
    final row =
        await (db.select(db.recipes)..where((r) => r.id.equals(id))).getSingle();
  ...
  test('JsonMapConverter round-trips correctly', () async {
    final id = await db.into(db.trips).insert(
          TripsCompanion.insert(
            ...
            status: 'probable',
            meta: const Value({'nested': 1, 'list': [1, 2]}),
          ),
        );
    final row =
        await (db.select(db.trips)..where((t) => t.id.equals(id))).getSingle();
  ...
  test('NullAwareTypeConverter returns null when column is null', () async {
    final id = await db.into(db.trips).insert(
          TripsCompanion.insert(title: 'No links', status: 'probable'),
        );
    final row =
        await (db.select(db.trips)..where((t) => t.id.equals(id))).getSingle();
